...
class MongoDB:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    raise SystemExit(
                        f"Database connection failed after "
    ...
    async def get_assistant(self, chat_id: int):
        ...
        if chat_id not in self.assistant:
            ...
            num = (
                doc["num"]
                if doc
                else await self.set_assistant(chat_id)
            )
    ...
    async def get_client(self, chat_id: int):
        ...
        if (
            hasattr(userbot, "one")
            and userbot.one in userbot.clients
        ):
            available_clients[1] = userbot.one

        if (
            hasattr(userbot, "two")
            and userbot.two in userbot.clients
        ):
            available_clients[2] = userbot.two

        if (
            hasattr(userbot, "three")
            and userbot.three in userbot.clients
        ):
    ...
    async def del_gban(
        ...
        if (
            hasattr(self, "gbanned_users")
            and user_id in self.gbanned_users
        ):
    ...
    async def get_group_for_channel(
        ...
        """Reverse lookup for channel play."""
        doc = await self.cache.find_one(
        ...
        if (
            doc
            and doc.get("_id", "").startswith("cplay_")
        ):
            group_id_str = doc["_id"].replace(
                "cplay_",
                "",
            )
    ...
    async def get_autoleave(
        ...
        """Get auto-leave status for a chat."""
        doc = await self.cache.find_one(
    ...
    async def get_autoplay(
        ...
        """Get autoplay status for a chat. Default is False."""
        doc = await self.cache.find_one(
    ...
    async def set_play_mode(
        ...
        if remove:
            ...
            await self.playmodedb.insert_one(
                {"_id": chat_id}
            )
    ...
    async def migrate_coll(self) -> None:
        ...
        logger.info(
            "🔄 Migrating users and chats from old collections..."
        )
        ...
        for user in ulist:
            try:
                ...
                logger.debug(
                    f"Skipping invalid user entry: {e}"
                )
        ...
        if musers:
            try:
                ...
                logger.debug(
                    "User migration bulk insert error "
                    f"(may be duplicate keys): {e}"
                )
        ...
        try:
            async for chat in self.chatsdb.find():
                try:
                    ...
                    logger.debug(
                        f"Skipping invalid chat entry: {e}"
                    )
        ...
        if mchats:
            try:
                ...
                logger.debug(
                    "Chat migration bulk insert error "
                    f"(may be duplicate keys): {e}"
                )
