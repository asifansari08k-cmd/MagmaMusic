...
# ============================================================
# SUPPRESS NON-CRITICAL MONGODB BACKGROUND ERRORS
# ============================================================
...
class MongoBackgroundFilter(logging.Filter):
    def filter(self, record):
        ...
        return not (
            ...
            in msg
            or (
                "AutoReconnect" in msg
                and "background task" in msg
            )
            or (
                "_OperationCancelled" in msg
                and "background task" in msg
            )
        )
...
class MongoDB:
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(
            ...
            try:
                ...
                await self.mongo.admin.command(
                    "ping"
                )
                ...
                await self.langdb.create_index(
                    "_id"
                )
    ...
    async def get_admins(
        ...
        from Anysnap.helpers._admins import (
            reload_admins,
        )
    ...
    async def _get_auth(
        ...
        if chat_id not in self.auth:
            ...
            doc = await self.authdb.find_one(
                {
                    "_id": chat_id
                }
            ) or {}
    ...
    async def get_assistant(
        ...
        if chat_id not in self.assistant:
            ...
            doc = await self.assistantdb.find_one(
                {
                    "_id": chat_id
                }
            )
    ...
    async def get_lang(
        ...
        if chat_id not in self.lang:
            ...
            doc = await self.langdb.find_one(
                {
                    "_id": chat_id
                }
            )
    ...
    async def get_play_mode(
        ...
        if chat_id not in self.play_mode:
            ...
            doc = await self.playmodedb.find_one(
                {
                    "_id": chat_id
                }
            )
    ...
    async def set_play_mode(
        ...
        if remove:
            ...
            if chat_id in self.play_mode:
                ...
                self.play_mode.remove(
                    chat_id
                )

            await self.playmodedb.delete_one(
                {
                    "_id": chat_id
                }
            )
    ...
    async def migrate_coll(
        ...
        if musers:
            ...
            try:
                ...
                await self.usersdb.insert_many(
                    musers,
                    ordered=False,
                )
        ...
        try:
            ...
            logger.debug(
                f"Error reading chats collection: {e}"
            )
        ...
        if mchats:
            ...
            try:
                ...
                await self.chatsdb.insert_many(
                    mchats,
                    ordered=False,
                )
    ...
    async def load_cache(
        ...
        logger.info(
            "✅ Cache loaded: "
            f"{len(self.chats)} chats, "
            f"{len(self.users)} users, "
            f"{len(self.blacklisted)} blacklisted."
        )